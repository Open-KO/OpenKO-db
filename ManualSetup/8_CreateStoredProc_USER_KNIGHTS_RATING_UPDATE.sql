...
INSERT INTO KUSER_RATING
    SELECT TOP 100
        USERDATA.strUserId, KNIGHTS.IDName, USERDATA.Loyalty
    FROM USERDATA
    INNER JOIN KNIGHTS ON
        USERDATA.Knights = KNIGHTS.IDNum
    WHERE
        USERDATA.Nation = 1 AND
        USERDATA.City <> 255 AND
...
INSERT INTO EUSER_RATING 
    ...
    INNER JOIN KNIGHTS ON
        USERDATA.Knights = KNIGHTS.IDNum
    WHERE
        USERDATA.Nation = 2 AND
        USERDATA.City <> 255 AND
        Authority NOT IN(0, 250, 255)
    ORDER BY
        USERDATA.Loyalty DESC,
        USERDATA.LoyaltyMonthly DESC
...
DECLARE @strKnightsName varchar(21)
DECLARE @nIndex smallint
DECLARE @nLoyalty int

SET @strCharID = NULL
SET @strKnightsName = NULL
SET @nIndex = 0
SET @nLoyalty = 0
...
DECLARE job1 CURSOR FOR
    SELECT
        nRank, strUserID, strKnightsName, nLoyalty
    FROM KUSER_RATING

OPEN job1
FETCH NEXT FROM job1
INTO @nIndex, @strCharID, @strKnightsName, @nLoyalty
WHILE @@fetch_status = 0 
BEGIN
    UPDATE USER_KNIGHTS_RANK SET
        strKarusUserID = @strCharID,
        strKarusKnightsName = @strKnightsName,
        nKarusLoyalty = @nLoyalty,
        isClaimedKarus = 0
    WHERE shIndex = @nIndex
...
BEGIN
    UPDATE USER_KNIGHTS_RANK SET
        strElmoUserID = @strCharID,
        strElmoKnightsName = @strKnightsName,
        nElmoLoyalty = @nLoyalty,
        isClaimedElmo = 0
    WHERE shIndex = @nIndex
